...
async function RadicarPropuesta(page) {
  try {
    ...
    await page.waitForSelector("span.menu-item-parent.ng-binding", {
      visible: true,
    });
...
async function Informacion_financiera(page) {
  ...
  await page.evaluate((Datos_Economicos) => {
    ...
    document.getElementById("activoCorrienteId0").value =
      Datos_Economicos.activoCorrienteId0;

    angular
      .element(document.getElementById("activoCorrienteId0"))
      .triggerHandler("change");

    document.getElementById("pasivoCorrienteId0").value =
      Datos_Economicos.pasivoCorrienteId0;

    angular
      .element(document.getElementById("pasivoCorrienteId0"))
      .triggerHandler("change");
    document.getElementById("activoTotalId0").value =
      Datos_Economicos.activoTotalId0;

    angular
      .element(document.getElementById("activoTotalId0"))
      .triggerHandler("change");

    document.getElementById("pasivoTotalId0").value =
      Datos_Economicos.pasivoTotalId0;
...
async function Certificado_Shapefile(page, Empresa, IdArea) {
  ...
  try {
    ...
    const ControladorDeCargaShapefile = await page.$(
  ...
  try {
    ...
    const ControladorCargaCertificado = await page.$(
  ...
  try {
    ...
    if (imagendeCaptcha == 0) {
      ...
      if (captchaIframeHandle) {
        ...
        if (!captchaFrame) {
          console.log(
            "⚠️ El contenido del iframe aún no está listo. Reintentando..."
          );
        } else {
          // Intentar detectar imágenes
          const imageTiles = await captchaFrame.$$("img");
          console.log(
            `Se encontraron ${imageTiles.length} imágenes en el desafío.`
          );
        }
  ...
  (async () => {
      ...
      try {
        ...
        if (mensajes.some((msg) => msg.includes("CELL_REOPENING_DATE"))) {
          ...
          try {
            ...
            const celdasNoDisponibles = await page.$$eval(
              "a.errorMsg",
              (links) => {
                return links
                  .filter((link) =>
            ...
            if (Band != 81) {
              ...
              const celdasNoDisponiblesLimpias = celdasNoDisponibles[0].map(
                (celda) => celda.trim()
              );
    ...
    try {
      ...
      try {
        ...
        const btnInfoTecnica = await page.$x(
          '//a[contains(.,"Información t")]'
        );
        await btnInfoTecnica[0].click();
    ...
    //   if (
    //     page.url() ===
    //     "https://annamineria.anm.gov.co/sigm/index.html#/p_CaaIataSummary"
    //   ) {
    ...
    //     "https://annamineria.anm.gov.co/sigm/index.html#/p_CaaIataAttachDocuments"
    //   ) {
    //     const posibleContinuar = await page.$x(
    //       '//span[contains(.,"Continuar")]'
    //     );
    //     if (posibleContinuar.length > 0) {
    //       console.log("⚠️ Se encontró el botón 'Continuar' en la página.");
    //       console.log([posibleContinuar]);
...
function Correo(Tipo, Area, Celda) {
  ...
  if (Tipo == 1) {
    ...
    msg = `Area Radicada  ${EquipoActual} ${Area} ${Empresa}`;
    Color = "#D4AF37";
    ...
    msg = `Rapido aparecio un recaptcha   ${EquipoActual}`;
...
const Areas = [
  ...
  {
    NombreArea: "18N05E04D14R",
    Referencia: "18N05E04D14R",
    Celdas: ["18N05E04D14R"],
  },
  /* {
      NombreArea: "prueba",
